...
const PROVIDER = "svelte-probe" as const;
...
type ProbeFailure = {
  ...
  error: {
    kind: "usage" | "fs" | "parse" | "policy" | "unknown";
    message: string;
    code?: string | null;
    details?: JsonValue;
  };
...
function printJson(
  value: ProbeSuccess | ProbeFailure | SelfCheckSuccess,
  exitCode = 0,
): never {
    ...
    printJson(
      {
        ...
        error: {
          kind: "usage",
          message: "Usage: bun run src/probe.ts <path-to-file.svelte> | --self-check",
          code: "USAGE",
        },
      },
      2,
      ...
      printJson(
        {
          ...
          error: {
            kind: "policy",
            message: "Probe target exceeds max allowed size",
            code: "FILE_TOO_LARGE",
            details: {
              bytes,
              maxFileBytes: MAX_FILE_BYTES,
