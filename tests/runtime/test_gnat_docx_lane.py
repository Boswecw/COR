...
class GnatDocxLaneRuntimeTests(unittest.TestCase):
    def test_docx_gnat_parallel_run_emits_schema_valid_ready_receipt(self) -> None:
        plan = plan_gnat_run(
            [GnatSourceInput(DOCX_READY_FIXTURE, media_type=DOCX_MEDIA_TYPE, source_ref="docx-note")],
            request_id="gnat-docx-ready",
        )
        ...
        self.assertEqual(result.receipts[0]["bounded_output"]["structures"]["metadata_fields"]["source_lane"], "docx_text")
        assert_schema_valid(self, result.receipts[0], schema_name="gnat-worker-receipt.schema.json")

    def test_docx_gnat_denial_is_visible_for_reviewed_docx(self) -> None:
        plan = plan_gnat_run(
            [GnatSourceInput(DOCX_REVIEWED_FIXTURE, media_type=DOCX_MEDIA_TYPE, source_ref="docx-reviewed")],
            request_id="gnat-docx-denied",
        )
    ...
    def test_docx_gnat_unavailable_is_visible_for_corrupt_docx(self) -> None:
        plan = plan_gnat_run(
            [GnatSourceInput(DOCX_CORRUPT_FIXTURE, media_type=DOCX_MEDIA_TYPE, source_ref="docx-corrupt")],
            request_id="gnat-docx-unavailable",
        )
    ...
    def test_docx_gnat_cache_identity_uses_docx_lane_contract(self) -> None:
        plan = plan_gnat_run(
            [GnatSourceInput(DOCX_READY_FIXTURE, media_type=DOCX_MEDIA_TYPE, source_ref="docx-note")],
            request_id="gnat-docx-cache",
        )
        identity = cache_identity_for_shard(plan.shards[0])

        self.assertEqual(identity.worker_type, "docx_text_syntax")
        self.assertEqual(identity.lane_contract_version, "local_file_docx_text.v1")

    def test_docx_gnat_persistence_records_cache_for_ready_docx(self) -> None:
        plan = plan_gnat_run(
            [GnatSourceInput(DOCX_READY_FIXTURE, media_type=DOCX_MEDIA_TYPE, source_ref="docx-note")],
            request_id="gnat-docx-persist",
        )
        ...
        self.assertEqual(cache_record["worker_type"], "docx_text_syntax")
