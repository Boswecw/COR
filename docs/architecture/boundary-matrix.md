...
| freshness and invalidation signaling | yes | consuming apps as truth authority | freshness-reporting authority only for Cortex-owned artifacts | low to medium | stale, degraded, partial_success | invalidation is surfaced through explicit stale posture and invalidation policy, not a separate workflow state |
